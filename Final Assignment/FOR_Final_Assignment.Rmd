...
b-) Rank countries by their mean `co2_per_cap`. What are top 20 emitters? How does this change from the one from question 5a?
...
d-) In light of the three plots above, what can you say about co2 emissions since 1990?
...
c-) Re-create the same plot as above (b), now as a histogram (tip: use a `geom_histogram()`).
...
Run a linear model with `co2_emissions` as your dependent variable. The independent variables should be `population` and `gdp`. Print a table and interpret the output.
