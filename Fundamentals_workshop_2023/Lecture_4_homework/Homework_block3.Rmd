...
a-) Plot a histogram showing the distribution of donations per year (Tip: you can use base R here `?hist()`)

```{r q1a, echo=TRUE, message=FALSE, warning=FALSE, include=FALSE}
hist(io_income_rs$year)
```
b-) Plot various histogram showing the distribution of donations per year in each issue area. What do we learn?
...
io_income_rs %>%
  ...
  ungroup()%>%
  slice_head(n=10)%>%
  ggplot(mapping=aes(x=reorder(donor, amount_nominal), #order donors acc. to $$$
...
io_income_rs %>%
  ...
  labs( x = NULL, y = "Total donations to issue area", 
        title = "Donations by the United States since 2000 by issue area", 
    subtitle=" composed by 4500 randomly sampled observations from dataset",
    caption = "Source: io_income_rs dataset")+
  theme(panel.background = element_rect("white", "black", .5, "solid"),
    panel.grid.major = element_line(color = "grey", size = 0.3, linetype = "solid"),
...
io_income_rs %>%
  ...
  labs( x = NULL, y = NULL, 
        title = "Distribution of donations by donor type in 2000 and 2020",
    subtitle=" composed by 4500 randomly sampled observations from dataset",
    caption = "Source: io_income_rs dataset")+
  theme(panel.background = element_rect("white", "black", .5, "solid"),
    panel.grid.major = element_line(color = "grey", size = 0.3, linetype = "solid"),
...
Create 2 scatter plots containing all donation by year per donor type. Please add a smoothed line to show the direction of the relationship. The smoothed lines should be colored"red".
