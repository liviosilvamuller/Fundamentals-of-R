...
1-) Dataset containing all 6130 official speeches by presidents since 1985.

2-) Subset of 2014 "amazonian statements”: two  sentences before, two sentences after "amazon".

3-) Location of each speech.

4-) Hand-coding and supervised machine learning.
...
random <- plm::plm(con_vs_all ~ km_to_manaus + election_year + def_year + AAI,
                   data = ama_model, model = "random", index = c("president"))
summary(random)
# Better, but not a good choice here, why? (tip: is this a balanced panel)
```

# Nominal outcome (dependent) variables, multinomial is the name of the game!
...
Above the reference categories are Pure Economic Integration in international settings?!
