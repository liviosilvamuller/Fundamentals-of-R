...
Let's plot life expectancy in time!
...
Do you think that life increased for across all continents though?
...
Let's start with a simple bar plot, one continent only at two different time points.
...
display.brewer.all(colorblindFriendly = TRUE, type = "qual") # "qual" stands for qualitative!
gapminder %>% 
  filter(year == 1957 | year == 2007) %>% # filter
  ggplot(aes(x = continent, y = lifeExp, fill = continent)) + # fill by continent
  geom_boxplot() + # box plot
  facet_wrap(~year) + # separate plots by year; creates 2 different plots in this case
...
```{r}
library(ggthemes) # This package has several cool themes!
gapminder %>% 
  filter(year == 2007) %>%
