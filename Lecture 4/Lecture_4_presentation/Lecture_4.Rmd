...
- Third (and finally), we improve what we see.
  - We can scale x in log because the range of gdpPercap is too big, and specified it in dollars with `scale_x_log10()`.
...
p<- p+
  ...
  labs( y="Life Expectancy", x=" GDP per Capita",
        title= "Life expectancy and GDP per capita along time",
...
--
  ...
  -The code below is creating a new variable with value "purple" for observations.
...
--
  ...
  - This means: `ggplot(mapping=aes(color=continent))`. Continent is a variable of the gapminder dataset.
